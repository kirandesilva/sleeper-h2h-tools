...
def draw_h2h_plot(h2h_df: DataFrame, write_dest=None) -> Figure:
    ...
    ----------
    h2h_df : DataFrame
        A DataFrame generated by `Standings.make_h2h_frame()`
    ...
    h2h_df = h2h_df.copy(deep=True).reset_index(drop=False, names=["Team"])\
        .rename(columns={"points": "Points"})

    colors = [["lightgrey"] * len(h2h_df)]
    for col in h2h_df.iloc[:,1:-1]:
        h2h_df[col] = h2h_df[col].astype(str)
        colors.append(h2h_df[col].map(map_color).to_list())

    colors.append(["lightgrey"] * len(h2h_df))

    h2h_df["Points"] = [round(pt, 2) for pt in h2h_df["Points"]]
    fig = Figure(
        data=[
            Table(
                columnwidth=[100] * len(h2h_df.columns[:-1]) + [80],
                header={
                    "values": h2h_df.columns.to_list(),
                    "line_color": "darkslategray",
                    "fill_color": "paleturquoise",
                    "align": "left"
                },
                cells={
                    "values": [h2h_df[col] for col in h2h_df],
                    "line_color": "darkslategray",
                    "fill_color": colors,
                    "height": 60,
                    "align": "left"
    ...
    new_c = colors
    new_c[0] = ["black"] * len(h2h_df)
    new_c[-1] = ["black"] * len(h2h_df)
